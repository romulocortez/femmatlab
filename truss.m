...
if strcmp(action,'initialize'),
   ...
   text(0,0,'Aperte o botão "Começar" para ver os modos de vibrar da ponte', ...
...
%    load truss.mat a x0 xy;
   ...
   x0 = 5*modeShapes; %matriz dos autovalores
   n = get(popupHndl,'Value');
   set(axHndl,'Userdata',play);
   numframes = 15;
   del = x0(:,n);
%    del = reshape([del' zeros(1,4)] ,2,10)';
   del = reshape([del(1:12)' zeros(1,2) del(13:21)' 0],2,gdl/2)';
   [xd,yd] = gplot(a,xy);
   cla;
   ...
   elseif xory_max == 2 % dim em y. > dim. em x
       ...
       text(max(xy(:,2))/2, max(xy(:,1))+2.5,'Modos de vibrar da Ponte Treliçada', ...
      'HorizontalAlignment','center');
   end
   patch([0 max(xy(:,1)) max(xy(:,1)) 0],...
                [min(xy(:,2))-0.3*L min(xy(:,2))-0.3*L min(xy(:,2))-0.7*L min(xy(:,2))-0.7*L]...
                ,[1 1 1])
   freqText = ['Frequência Natural de Excitação :',num2str(freq(n)),' Hz'];
   text(max(xy(:,1))/2, min(xy(:,2))-L/2,freqText,...
           'HorizontalAlignment','center');
